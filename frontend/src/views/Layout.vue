  ...
  <div class="platform-shell">
    ...
    <aside v-if="mobileMenuOpen" class="mobile-nav-drawer" role="dialog" aria-modal="true" aria-labelledby="mobile-nav-title" @keydown.esc="closeMobileMenu">
      <header class="mobile-nav-head">
        <div><span>工作空间</span><strong id="mobile-nav-title">导航菜单</strong></div>
    ...
    <div class="platform-main">
      <header class="platform-topbar">
        ...
        <div class="topbar-contract">
          <span>模拟环境</span><strong>逻辑模拟</strong><i></i>
        </div>
      </header>
...
.platform-sidebar { width: 250px; min-width: 250px; min-height: 100vh; padding: 22px 14px 16px; display: flex; flex-direction: column; background: #1f2926; color: #eef2ed; transition: width .2s, min-width .2s; }
...
.brand-glyph { width: 42px; height: 42px; position: relative; flex: 0 0 42px; border: 1px solid #8da79a; border-radius: 50%; }
.brand-glyph i { position: absolute; inset: 10px 5px; border: 1px solid #b4d1c2; border-radius: 50%; transform: rotate(35deg); }
.brand-glyph i:nth-child(2) { transform: rotate(-35deg); }
.brand-glyph b { position: absolute; left: 17px; top: 17px; width: 7px; height: 7px; border-radius: 50%; background: #72a88d; }
...
.nav-index { width: 30px; color: #9dc6ae; font: 600 .68rem ui-monospace, monospace; }
...
.status-light { width: 8px; height: 8px; margin-top: 4px; border-radius: 50%; background: #72a88d; box-shadow: 0 0 0 4px rgba(114,168,141,.12); }
...
.platform-topbar { min-height: 78px; padding: 15px clamp(22px, 4vw, 54px); border-bottom: 1px solid var(--lz-line); display: flex; align-items: center; justify-content: space-between; background: rgba(250,250,248,.94); }

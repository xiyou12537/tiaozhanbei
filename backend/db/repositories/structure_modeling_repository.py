...
class StructureModelingRepository:
    ...
    def get_workflow_resources(self, workflow_id: str, owner_user_id: int) -> dict[str, Any] | None:
        ...
        try:
            ...
            quantum_closure_benchmarks = (
                ...
                .filter(
                    QuantumClosureBenchmarkRecord.workflow_id == workflow_id,
                    QuantumClosureBenchmarkRecord.owner_user_id == owner_user_id,
                )
                .order_by(QuantumClosureBenchmarkRecord.created_at.asc())
                .all()
            )
            benchmark_cases = (
            ...
            return {
                ...
                "quantum_closure_benchmarks": quantum_closure_benchmarks,
    ...
    def list_electronic_structure_candidates(
        self,
        quantum_region_id: str,
        owner_user_id: int,
    ) -> list[ElectronicStructureCandidateRecord]:
        return self._read(
            lambda session: session.query(ElectronicStructureCandidateRecord)
            .filter(
                ElectronicStructureCandidateRecord.quantum_region_id == quantum_region_id,
                ElectronicStructureCandidateRecord.owner_user_id == owner_user_id,
            )
            .order_by(ElectronicStructureCandidateRecord.created_at.asc())
            .all()
        ) or []
    ...
    def get_selected_research_benchmark_candidate_by_source_id(
        self,
        owner_user_id: int,
        source_candidate_id: str,
    ) -> ResearchBenchmarkCandidateRecord | None:
        """Load a source candidate only when the owner has selected it for a workflow."""
        return self._read(
            lambda session: session.query(ResearchBenchmarkCandidateRecord)
            .join(
                ResearchBenchmarkSelectionRecord,
                ResearchBenchmarkSelectionRecord.candidate_id == ResearchBenchmarkCandidateRecord.candidate_id,
            )
            .filter(
                ResearchBenchmarkSelectionRecord.owner_user_id == owner_user_id,
                ResearchBenchmarkCandidateRecord.source_candidate_id == source_candidate_id,
            )
            .order_by(ResearchBenchmarkSelectionRecord.created_at.desc())
            .first()
        )

    def get_research_benchmark_selection_for_workflow(
        self,
        workflow_id: str,
        owner_user_id: int,
    ) -> ResearchBenchmarkSelectionRecord | None:
        return self._read(
            lambda session: session.query(ResearchBenchmarkSelectionRecord)
            .filter(
                ResearchBenchmarkSelectionRecord.workflow_id == workflow_id,
                ResearchBenchmarkSelectionRecord.owner_user_id == owner_user_id,
            )
    ...
    def create_quantum_closure_benchmark(self, values: dict[str, Any]) -> QuantumClosureBenchmarkRecord:
        return self._write(lambda session: self._create(session, QuantumClosureBenchmarkRecord, values))

    def get_quantum_closure_benchmark(
        self,
        closure_benchmark_id: str,
        owner_user_id: int,
    ) -> QuantumClosureBenchmarkRecord | None:
        return self._read(
            lambda session: self._owned(
                session,
                QuantumClosureBenchmarkRecord,
    ...
    def list_quantum_closure_benchmarks(
        self,
        workflow_id: str,
        owner_user_id: int,
    ) -> list[QuantumClosureBenchmarkRecord]:
        return self._read(
            lambda session: session.query(QuantumClosureBenchmarkRecord)
            .filter(
                QuantumClosureBenchmarkRecord.workflow_id == workflow_id,
                QuantumClosureBenchmarkRecord.owner_user_id == owner_user_id,
            )
            .order_by(QuantumClosureBenchmarkRecord.created_at.desc())
            .all()
        ) or []
